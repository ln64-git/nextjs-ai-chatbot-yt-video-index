...
export const indexYouTubeChannel = tool({
  description:
    "Index a YouTube channel for semantic search. This will fetch all videos from the channel, extract transcripts, generate keywords, and create vector embeddings for comprehensive content search. Use this when the user provides a YouTube channel link and wants to index it.",
  inputSchema: z.object({
    ...
    maxVideos: z
      ...
      .optional()
      .describe(
        "Maximum number of videos to index (optional, defaults to all videos)"
      ),
    confirmIndexing: z
      .boolean()
      .optional()
  ...
  execute: async ({ channelUrl, channelName, maxVideos, confirmIndexing }) => {
    try {
      ...
      indexer
        .indexChannel(channelUrl, displayName, maxVideos)
        .catch((error) => {
